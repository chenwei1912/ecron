...
TEST_CASE("test Buffer read/write", "[BufferReadWrite]") {
    ...
    REQUIRE(ecron::Buffer::InitialSize == buffer.writable_bytes());

    const std::string str("hello");
    buffer.write(str);
    REQUIRE(str.size() == buffer.readable_bytes());
    REQUIRE(ecron::Buffer::InitialSize - str.size() == buffer.writable_bytes());
    ...
    REQUIRE(ecron::Buffer::InitialSize - 2 * str.size() == buffer.writable_bytes());
...
TEST_CASE("test buffer grow", "[BufferGrow]") {
    ...
    REQUIRE(ecron::Buffer::InitialSize - 400 == buffer.writable_bytes());
...
TEST_CASE("test buffer inside grow", "[BufferInsideGrow]") {
    ...
    REQUIRE(ecron::Buffer::InitialSize - 800 == buffer.writable_bytes());
    ...
    REQUIRE(ecron::Buffer::InitialSize - 600 == buffer.writable_bytes());
...
TEST_CASE("test buffer move", "[BufferMove]") {
    ...
    ecron::Buffer newbuf(std::move(buffer));
