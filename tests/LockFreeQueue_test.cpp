...
TEST_CASE("test LockFreeQueue", "[LockFreeQueue]") {

    ecron::LockFreeQueue<int> queue;
    //ecron::block_queue<int> queue;
    queue.init();
    ...
    for (size_t i = 0; i < cn; ++i)
    {
        cthreads.emplace_back([&, i](){
            ...
            while (true) {
                ...
                if (flag) {
                    if (val > num || val <= 0)
                        std::cout << "thread: " << i << " pop: " << val << std::endl;
                    ++count[val];
                    pop_cnt.fetch_add(1);
                }
                else if (cnt == pn) // no push thread
                    break;
            }
